...
<div class="max-w-2xl mx-auto">
    <h1 class="text-4xl font-black text-[#422168] text-center mb-8">Подтверждение email</h1>
    ...
    @if(session('warning'))
        <div class="bg-yellow-100 text-yellow-800 p-4 rounded-xl font-bold mb-6 border-l-8 border-yellow-500">
            {{ session('warning') }}
        </div>
    @endif
    ...
    <div class="bg-white p-8 rounded-3xl shadow-xl border-4 border-[#E8FC8C]">
        <div class="text-center mb-6">
            ...
            <h2 class="text-2xl font-black text-[#422168] mb-2">Проверьте вашу почту</h2>
            <p class="text-sm text-gray-600">
                Мы отправили 6-значный код на адрес:<br>
                <strong class="text-[#422168]">{{ Auth::user()->email }}</strong>
            </p>
        </div>

        <form action="{{ route('profile.verify-email.confirm') }}" method="POST" class="space-y-5">
            @csrf
            <div>
        ...
        <form action="{{ route('profile.verify-email.regenerate') }}" method="POST" class="mt-4">
            @csrf
