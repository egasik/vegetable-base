@extends('layouts.app')
@section('title', 'Подтверждение смены пароля')

@section('content')
<div class="max-w-2xl mx-auto">
    <h1 class="text-4xl font-black text-[#422168] text-center mb-8">Подтверждение смены пароля</h1>
    ...
    <div class="bg-white p-8 rounded-3xl shadow-xl border-4 border-[#E8FC8C]">
        <div class="text-center mb-6">
            <div class="w-20 h-20 mx-auto bg-[#CAF204] rounded-full flex items-center justify-center text-4xl mb-4">🔐</div>
            <h2 class="text-2xl font-black text-[#422168] mb-2">Проверьте почту</h2>
            <p class="text-sm text-gray-600">
                Код отправлен на ваш email:<br>
                <strong class="text-[#422168]">{{ Auth::user()->email }}</strong>
            </p>
        </div>

        <form action="{{ route('profile.password-change.confirm') }}" method="POST" class="space-y-5">
            @csrf
            <div>
        ...
        <form action="{{ route('profile.password-change.regenerate') }}" method="POST" class="mt-4">
            @csrf
